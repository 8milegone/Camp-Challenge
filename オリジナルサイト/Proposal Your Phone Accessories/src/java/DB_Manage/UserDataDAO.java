...
public class UserDataDAO {
    ...
    /**
     * ユーザーデータの挿入処理を行う。現在時刻は挿入直前に生成します。
     * @param ud 
     * @throws SQLException 
     */
    public void UserInsert(UserDataDTO ud) throws SQLException {
        ...
        try{
            ...
            st = con.prepareStatement("INSERT INTO user_t(name,password,mail,address,newDate,terminal) VALUES(?,?,?,?,?,?)");
            st.setString(1, ud.getName());
            st.setString(2, ud.getPassword());
            st.setString(3, ud.getMail());
            st.setString(4, ud.getAddress());     
            st.setTimestamp(5, new Timestamp(System.currentTimeMillis()));
            st.setString(6, ud.getTerminal());
            st.executeUpdate();
    ...
    public UserDataDTO UserSearch(UserDataDTO ud) throws SQLException {
        ...
        try{
            ...
            rs.next();
            //検索結果を参照するためにUserDTO()からインスタンス化して利用
            ...
            resultUd.setTerminal(rs.getString(9));
    ...
    /**
     * ユーザーIDによるデータの更新処理を行う。更新日時は更新する直前に生成します。
     * @param ud 対応したデータを保持しているJavaBeans
     * @throws SQLException 呼び出し元にcatchさせるためにスロー
     */
    public void UserUpdate(UserDataDTO ud) throws SQLException{
        ...
        try{
            con = DBManager.getConnection();
            //SQLへのコードを記入
            String sql = "UPDATE user_t SET name=?, password=?, mail=?, address=?, newDate=? ,terminal=? WHERE userID= ?" ;
    ...
    void updateTotal(int parseInt, int userID) {
